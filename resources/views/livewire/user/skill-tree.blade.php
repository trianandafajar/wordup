...
@section('content')
    @foreach ($units as $unitData)
        <div class="w-full max-w-lg mb-4">
            <div class="rounded-2xl {{ $unitData['lessons']->every('status', 'completed') ? 'bg-brand-500' : 'bg-gray-300 text-gray-600' }} text-white p-4 shadow-md flex items-center justify-between gap-4 mb-8">
                <div>
        ...
        <div class="flex flex-col items-center gap-0 w-full mb-8" style="padding-bottom: 2rem;">
            @foreach ($unitData['lessons'] as $index => $lesson)
                @php
                    ...
                    $position = $index % 4;
                    $mlClass = $position === 1 ? 'self-start ml-8 sm:ml-16' : ($position === 3 ? 'self-end mr-8 sm:mr-16' : 'self-center ml-0');
                @endphp
                ...
                <div class="{{ $mlClass }} mb-0">
                    <button
                        @if (!$isLocked)
                            onclick="window.location.href='{{ route('user.lesson.practice', $lesson['id']) }}'"
                        @else
                            disabled
                        @endif
